...
def create_pku_dataloader_from_dataset(tokenizer, dataset, fraction=1.0, batch_size=64):
    ...
    def preproccess(examples):
        ...
        #         # Truncate the list
        #         normalized_dict[key] = value[:input_len]
        #     else:
        #         normalized_dict[key] = value + [[0]] * (input_len - len(value))
    ...
    #     ],
    # )
    print("Start preproccessing dataset")
    dataset = preproccess(dataset[:70000])
...
def compute_reverse_kl(pretrained_model, current_model, batch, device):
    ...
    Returns:
       The KL loss.
    """
    # print(f"batch input_id's shape: {batch['input_ids'].shape}")
    normal_outputs = current_model(
        batch["input_ids"].to(device),
        attention_mask=batch["attention_mask"].to(device)
    )
    # print(f"normal_outputs's shape: {normal_outputs.logits.shape}")
    with torch.no_grad():
        pretrained_outputs = pretrained_model(
            batch["input_ids"].to(device),
            attention_mask=batch["attention_mask"].to(device)
        )
    # print(f"pretrained_outputs's shape: {pretrained_outputs.logits.shape}")

    # Q: current model; P: pretrained model.
    normal_logits = torch.clamp(normal_outputs.logits, min=-10, max=10)
    pretrained_logits = torch.clamp(pretrained_outputs.logits, min=-10, max=10)

    prob_q = torch.nn.functional.softmax(normal_logits, dim=-1)
    prob_p = torch.nn.functional.softmax(pretrained_logits, dim=-1)

    # torch.save(prob_q, "prob_q.pt")
    # torch.save(prob_p, "prob_p.pt")
    
    # print(f"Value: {prob_q}, {prob_p}")
    # print(prob_q.shape, prob_p.shape)

    # Negative KL divergence: sum(Q * log(Q/P))
    epsilon = 1e-12
    loss =  + (prob_p * torch.log((prob_p + epsilon) / (prob_q + epsilon))).sum(-1).mean()
...
def get_rand_ans_loss(operation, bad_batch, tokenizer, normal_ans, model, K=5, device="cuda:0"):
    """
    ...
    # GD on answer.
    random_loss = get_answer_loss(operation, batch_random, model, device=device)
