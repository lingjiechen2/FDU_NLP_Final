...
set_seed(32)
TOKEN = "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"

def main(model_name, model_type, task_vector_saving_path = None, task_vector_exist = False, if_original_model = False, is_quantized=True):
    """
    ...
    if model_type == "facebook/opt-2.7b":
        ...
        model_path = "/remote-home/miintern1/SKU/harmful_unlearn/trained_models/task_vector/OPT_step_1000_batch_8"
    ...
    if if_original_model:
        if model_type == "facebook/opt-2.7b":
            ...
            if is_quantized:
                ...
                model = AutoModelForCausalLM.from_pretrained(model_type,token=access_token)
        ...
        if task_vector_exist == False:
            ...
            model = AutoModelForCausalLM.from_pretrained(PATH, quantization_config=bnb_config, local_files_only=True) if is_quantized else AutoModelForCausalLM.from_pretrained(PATH, local_files_only=True)
            if model_type == "facebook/opt-2.7b":
                pretrained_model = AutoModelForCausalLM.from_pretrained(model_type,
                                                                        quantization_config=bnb_config) if is_quantized else AutoModelForCausalLM.from_pretrained(model_type)
            else:
                pretrained_model = AutoModelForCausalLM.from_pretrained(model_type,
                                                                        quantization_config=bnb_config,
                                                                        token=access_token) if is_quantized else AutoModelForCausalLM.from_pretrained(model_type, token=access_token)
            ...
            print(f"Loading trained task vector model from {model_name}")
            if model_type == "facebook/opt-2.7b":
                model = AutoModelForCausalLM.from_pretrained(model_name,
                                                         quantization_config=bnb_config,
                                                         local_files_only=True) if is_quantized else AutoModelForCausalLM.from_pretrained(model_name, local_files_only=True)
            else:
                print("Mergeing LoRA weight...")
                tokenizer = AutoTokenizer.from_pretrained(model_type, token=access_token)
                model = AutoModelForCausalLM.from_pretrained(model_type,
                                                         quantization_config=bnb_config,
                                                         local_files_only=True) if is_quantized else AutoModelForCausalLM.from_pretrained(model_type,
                                                         local_files_only=True)
                model = PeftModel.from_pretrained(model, model_name, use_auth_token=access_token)
...
if __name__ == '__main__':
    ...
    for i in range(100, 1200, 100):
        # task_vector_saving_path = f"/remote-home/miintern1/SKU/harmful_unlearn/trained_models/task_vector/OPT_step_1000_batch_2_{i}"
        task_vector_saving_path = f"/remote-home/miintern1/SKU/harmful_unlearn/trained_models/LLAMA_7B_step_1000_batch_2_lr_2e-5_{i}/adapter_model.safetensors"
        print(f"Running task vector generation for {i} Step Model")
        main(task_vector_saving_path, model_type, task_vector_exist = True, if_original_model=False, is_quantized=False)